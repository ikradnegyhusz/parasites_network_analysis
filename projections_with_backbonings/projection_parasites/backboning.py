...
def test_densities(table, start, end, step):
   ...
   for s in steps:
      ...
      G = nx.from_pandas_edgelist(edge_table, source="src", target="trg", create_using=nx.Graph())
      ...
      result.append([s, nodes, (100.0 * nodes) / onodes, edges, (100.0 * edges) / oedges, avgdeg, avgdeg / oavgdeg, largest_size,n_connected_components])
   return result

def noise_corrected(table, undirected = True, return_self_loops = False, calculate_p_value = False):
...
def doubly_stochastic(table, undirected=True, return_self_loops=False):
...
def high_salience_skeleton(table, undirected = True, return_self_loops = False):
...
def naive(table, undirected = True, return_self_loops = False):
